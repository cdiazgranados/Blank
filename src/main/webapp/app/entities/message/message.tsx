  ...
  return (
    <div>
      <h2 id="message-heading" data-cy="MessageHeading">
        <Translate contentKey="jiveApp.message.home.title">Messages</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jiveApp.message.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/message/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jiveApp.message.home.createLabel">Create new Message</Translate>
          </Link>
        </div>
      </h2>
        ...
        {messageList && messageList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="jiveApp.message.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {messageList.map((message, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{message.text}</td>
                  <td>{message.membership ? <Link to={`/membership/${message.membership.id}`}>{message.membership.id}</Link> : ''}</td>
                  <td>{message.channel ? <Link to={`/channel/${message.channel.id}`}>{message.channel.id}</Link> : ''}</td>
                  <td>{message.membership ? <Link to={`/membership/${message.membership.id}`}>{message.membership.id}</Link> : ''}</td>
                  <td>{message.channel ? <Link to={`/channel/${message.channel.id}`}>{message.channel.id}</Link> : ''}</td>
                  <td>
                    {message.directMessage ? (
                      <Link to={`/direct-message/${message.directMessage.id}`}>{message.directMessage.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jiveApp.message.home.notFound">No Messages found</Translate>
            </div>
          )
        )}
      </div>
